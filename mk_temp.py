...
ctlg_path = os.path.join(out_dir, '%s_temp.ctlg'%net)
pha_path  = os.path.join(out_dir, '%s_temp.pha'%net)
out_ctlg = open(ctlg_path, 'w')
out_pha  = open(pha_path, 'w')
# make sta dir
sta_file = '/data3/XJ_SAC/header/station.dat'
...
for phase in phases:
  ...
  # org picks
    ...
    # filter bad pick TODO
    if float(phase.split(',')[-1])==0\
    or float(phase.split(',')[-2])==0: continue
